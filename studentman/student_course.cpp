    ...
    ui(new Ui::student_course)
{
    ...
    this->setStyleSheet("student_course{border-image:url(:/jpg/3.jpg);}");
}
...
{
//    SELECT cno from(select sno ,cno from score where sno='st0001')as a where cno='C0001'
    QString cno=ui->lineEdit->text();
    QSqlQuery query;
    QString sql_1=QString("SELECT cno from(select sno ,cno from score where sno='%1')as a where cno='%2'").arg(ID).arg(cno);
    QString sql_2=QString("SELECT cno from course where cno='%1'").arg(cno);
    query.exec(sql_2);
    query.first();
    if(query.value(0).isNull()==0){
        ...
        if(query.value(0).isNull()==1)
        {
                ...
                QString sql;
                sql = QString("insert into score VALUES ('%1','%2',NULL)")
                                .arg(ID).arg(cno);
...
void student_course::on_outBtn_clicked()
{
    ...
    if(query.value(0).isNull()==0){
        ...
        if(query.value(0).isNull()==0)
        {
                ...
                QString sql;
                sql = QString("DELETE FROM score where sno='%1' and cno='%2'")
                                .arg(ID).arg(cno);
...
void student_course::on_backButton_clicked()
{
    this->hide();
}
